...
版本：`260919-localfix8`。本次以用户指定仓库 `luluarthub/wx_channels_download` 的 `124f044235bc706fe796c043a14ff442a150ae5d` 为基线，迁移 `260916-localfix5` 的本地修复。旧源码保留；原安装目录在备份后切换新版，并保留用户配置和数据库。

## 合并内容

- 保留线上新版界面、多平台适配、直播、更新与 MCP 功能。
- 视频号下载以点击时的卡片身份定位，遇到身份不明确时停止，避免滚动后下载旧视频。
- 兼容微信长标题的折叠文本结构，拼接完整描述时排除“展开／收起”等控件文字，修复按钮存在但无法识别当前视频的问题；继续要求完整、唯一的卡片匹配。
...
- 修复 Windows 注册表与实际连接配置不同步时，微信没有下载按钮的问题；同一事务同步 WinINet 连接配置与旧式代理开关，回读验证，并保留事务回滚和实例归属保护。关闭程序和恢复网络脚本也同步两套状态。
...
go run ./tools/runtimeverify -exe dist/localfix8/wx_video_download.exe -output F:/workspace/localfix8-runtime-check
```
